...
class SiriProxy::Plugin::Example < SiriProxy::Plugin
  ...
  listen_for %r{isbn (\d+)}i do |d_isbn|
    say "Adding ISBN #{d_isbn}..."
...
=begin
  ...
  listen_for %r{/test siri proxy}i do
  ...
  listen_for %r{confirm state change}i, within_state: :some_state do #this only gets processed if you're within the :some_state state!
  ...
  listen_for %r{siri proxy test question}i do
  ...
  listen_for %r{siri proxy number ([0-9,]*[0-9])}i do |number|
